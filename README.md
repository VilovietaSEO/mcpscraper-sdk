...
List first, use one exact returned path to read, and set `depositToLibrary: true` only when the complete selected file should be preserved in the tenant Library vault.

<details open><summary>Node.js</summary>

```ts
const archiveUrl = 'https://github.com/octocat/Hello-World/archive/refs/heads/master.zip'
const listing = await client.tools.web.archiveRead({ url: archiveUrl })
const file = await client.tools.web.archiveRead({
  url: archiveUrl,
...
```python
archive_url = "https://github.com/octocat/Hello-World/archive/refs/heads/master.zip"
listing = client.tools.web.archive_read(url=archive_url)
file = client.tools.web.archive_read(
    url=archive_url,
    path="Hello-World-master/README",
...
```bash
ARCHIVE_URL='https://github.com/octocat/Hello-World/archive/refs/heads/master.zip'
mcpscraper tools call archive_read --args "{\"url\":\"$ARCHIVE_URL\"}" --json
mcpscraper tools call archive_read \
  --args "{\"url\":\"$ARCHIVE_URL\",\"path\":\"Hello-World-master/README\",\"depositToLibrary\":true}" \
  --json
```
</details>
...
curl https://mcpscraper.dev/mcp \
  ...
  -H "content-type: application/json" \
  -H "accept: application/json, text/event-stream" \
  --data '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"archive_read","arguments":{"url":"https://github.com/octocat/Hello-World/archive/refs/heads/master.zip","path":"Hello-World-master/README","depositToLibrary":true}}}'
```
</details>
...
Every one of memory.mcpscraper.dev's 145 tools (governed capture, separate Research and CRM records, tags, graph traversal, search, vaults, tables, scheduled actions, webhooks, and more — see [`contracts/memory.tools.json`](./contracts/memory.tools.json)) is callable from `mcpscraper-sdk` alone, dispatched through `POST /memory/mcp-call`. mcpscraper.dev auto-provisions a memory identity for your account on first use — no separate `mk_...` key required.
...
The legacy `memoryTools`/`memory_tools.call_tool(...)` bridge remains available for compatibility. New integrations should use `client.tools`, which provides typed methods for all 369 unified tools in both Node and Python.

## Scheduled results and artifact templates

Scheduled Actions now have typed contracts for a result-first inbox and safe, versioned report presentation. `No artifact` affects only rendered HTML; it does not disable normal Memory-note output. Selecting a saved template always pins its exact immutable version.

```ts
const { templates } = await client.tools.schedule.listArtifactTemplates({ status: 'active' })
const { items, nextCursor } = await client.tools.schedule.listScheduledRuns({
  view: 'inbox',
  limit: 30,
})
const run = await client.tools.schedule.getScheduledRun({ runId: items[0].runId })
const link = await client.tools.schedule.createScheduledRunViewLink({
  runId: run.run.runId,
  artifactId: run.run.artifactOutputs[0].artifactId,
  expiresInDays: 7,
})
```
...
runs = client.tools.schedule.list_scheduled_runs(view="inbox", limit=30)
run = client.tools.schedule.get_scheduled_run(run_id=runs.items[0]["runId"])
link = client.tools.schedule.create_scheduled_run_view_link(
    run_id=run.run["runId"],
    artifact_id=run.run["artifactOutputs"][0]["artifactId"],
    expires_in_days=7,
)
```

Run IDs and cursors are opaque: copy and return them unchanged. View-link URLs are returned only on creation and grant read-only access to one artifact until expiry or revocation.
...
Pass the complete `continuation` object returned by a partial generic export unchanged to resume the exact original range. Provider content is untrusted data. For Gmail full-fidelity bodies, raw EML, and actual attachment bytes, use the named workflow below.
...
## Complete Gmail workflows

The generated `client.tools.connections` namespace exposes ten first-class Gmail methods for normalized search and complete reads, opaque attachment access, immutable 24-hour selections, complete export, reversible bulk management, separate permanent deletion, and reviewed/resumable Memory import. Keep every selection's `selectionId`, `selectionSha256`, and `count` together; retries reuse one idempotency key. `gmail_import_status` observes only and never resumes writes.

```ts
const selection = await client.tools.connections.gmailPrepareSelection({
  connectionId: 'gmail_conn_123',
  purpose: 'memory_import',
  source: { kind: 'query', query: 'has:attachment newer_than:30d' },
})
const plan = await client.tools.connections.gmailPrepareMemoryImport({
  connectionId: 'gmail_conn_123',
  selectionId: selection.selection!.selectionId,
  selectionSha256: selection.selection!.selectionSha256,
  attachmentPolicy: 'preserve_all',
})
const ingest = await client.tools.connections.gmailImportToMemory({
  importPlanId: plan.plan!.importPlanId,
  idempotencyKey: 'mailbox-archive-2026-08-27',
})
```

See [`examples/gmail-complete-workflow.mjs`](./examples/gmail-complete-workflow.mjs) for search → full read/attachments → preview → import → resume/status, and [`examples/gmail-bulk-manage.mjs`](./examples/gmail-bulk-manage.mjs) for a reversible label add/remove receipt. Permanent delete is deliberately excluded from the examples: it is irreversible and requires a separately reviewed disposable-only selection plus `confirmPermanentDelete:true`.
...
For X-Ray identity reporting, [`examples/xray-journeys.mjs`](./examples/xray-journeys.mjs) demonstrates confirmed-only pagination and separately labeled `candidate_assisted` best-guess evidence. Best-guess results are analysis-only and must not feed CRM or advertising actions. [`examples/xray-attribution.mjs`](./examples/xray-attribution.mjs) reads the provider-neutral attribution methodology and truthful view-evidence status, with structured survey creation kept behind an explicit draft gate.
...
`mcpscraper-cli` keeps 7 ergonomic shortcuts (`search`, `scrape`, `crawl`, `map`, `maps-search`, `memory search`, `memory list-vaults`) and also reaches all 369 tools through `mcpscraper tools list`, `mcpscraper tools describe <name>`, and `mcpscraper tools call <name> --args '<json>'`. Tools marked destructive require `--yes`. Every command reads `MCPSCRAPER_API_KEY` from the environment or `--api-key`.
...
- [`contracts/mcp.tools.json`](./contracts/mcp.tools.json) — canonical release-derived contract for all 369 tools. Source of truth for every Node/Python typed namespace, CLI catalog, and [cURL catalog](./docs/curl-tools.md).
...
- [`contracts/memory.tools.json`](./contracts/memory.tools.json) — tool manifest (name, description, input/output JSON Schema per tool) for memory.mcpscraper.dev's 145 tools. Source of truth for `mcpscraper-memory-sdk` (Node and Python) and `mcpscraper-sdk`'s `memoryTools`/`memory_tools` bridge.
