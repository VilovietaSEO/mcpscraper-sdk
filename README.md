...
The legacy `memoryTools`/`memory_tools.call_tool(...)` bridge remains available for compatibility. New integrations should use `client.tools`, which provides typed methods for all 160 unified tools in both Node and Python, including all 87 memory tools.
...
- [`contracts/mcp.tools.json`](./contracts/mcp.tools.json) — canonical live-derived contract for all 160 tools. Source of truth for every Node/Python typed namespace, CLI catalog, and [cURL catalog](./docs/curl-tools.md).
