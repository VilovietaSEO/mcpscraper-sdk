...
- Generate Node, Python, CLI, and cURL Search contracts from MCP Scraper 0.95.0. `search_serp` accepts `week` or `month` for date-filtered organic results; filtered searches cost 35 Credits per delivered page and follow Google's pagination when two pages are requested. Unfiltered searches keep their normal provider priority and use the browser only as the final backup.
- Publish the updated Node scraper and Memory SDKs as 0.51.0, CLI as 0.50.0, Python scraper SDK as 0.49.0, and Python Memory SDK as 0.47.0. Callers who passed `day` or `year` to ordinary Search should use `week` or `month`.

## [0.50.1] - 2026-09-25 — targets mcp-scraper 0.94.1

### Changed

- Raise the generated Maps place review request maximum from 500 to 1,000 in Node, Python, CLI, and the REST contract. A 1,000-review declaration has a 1,060-Credit maximum hold; completed runs settle on unique reviews actually saved.
- Release Node scraper and Memory SDKs 0.50.1, CLI 0.49.1, Python scraper 0.48.1, and Python Memory SDK 0.46.1 with the 364-tool MCP Scraper 0.94.1 contract.
...
- Describe two-page Google Search as organic-only, with a partial one-page result when page 2 cannot be delivered. The generated Node, Python, CLI, and cURL contracts now show the delivered-page billing and rich-feature limit.

### Changed

- Release Node scraper and Memory SDKs 0.49.1, CLI 0.48.1, Python scraper 0.47.1, and Python Memory SDK 0.45.1 with the 0.93.1 server contract.

## [0.49.0] - 2026-09-24 — targets mcp-scraper 0.93.0

### Added

- Expose the Google Search light/full mode input and structured same-page SERP feature results in the generated Node, Python, CLI, and cURL contracts. Both modes default to one page; callers request `pages: 2` explicitly.

### Changed

- Document 20 Credits per delivered light page when the primary search succeeds and 35 Credits per delivered full or backup page. Prepared Node `mcpscraper-sdk` and `mcpscraper-memory-sdk` 0.49.0, `mcpscraper-cli` 0.48.0, Python `mcpscraper-sdk` 0.47.0, and Python `mcpscraper-memory-sdk` 0.45.0 for the server 0.93.0 contract.

## [0.48.1] - 2026-09-24 — targets mcp-scraper 0.92.1

### Changed

- Updated the `search_serp` tool contract, typed clients, CLI catalog, REST schema, and public examples to show 20 Credits per delivered page, or 35 Credits per delivered page when the backup supplies the result. The higher requested-page amount is held and settled after delivery.
- Prepared Node `mcpscraper-sdk` 0.48.1, `mcpscraper-cli` 0.47.1, and Python `mcpscraper-sdk` 0.46.1 for the MCP Scraper 0.92.1 pricing contract.
...
- Bound the generated contract to the 0.67.1 server patch, which preserves readable JSON while also returning the same successful Commons image result through MCP `structuredContent`.
...
- Prepared `mcpscraper-memory-sdk` 0.31.3, `mcpscraper-sdk` 0.30.3, `mcpscraper-cli` 0.29.3, Python `mcpscraper-sdk` 0.29.3, and Python `mcpscraper-memory-sdk` 0.28.3.
