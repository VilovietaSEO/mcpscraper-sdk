...
Official TypeScript/JavaScript clients for all 373 tools at [mcpscraper.dev](https://mcpscraper.dev) plus the direct 121-tool [memory.mcpscraper.dev](https://memory.mcpscraper.dev) API.
...
The direct Memory contract contains 121 tools. `MemoryClient` preserves the established compatibility namespaces and adds the source-generated `assistant` context-packet namespace. `McpToolsClient` is generated from [`contracts/mcp.tools.json`](../../contracts/mcp.tools.json) and contains all 373 unified tools. Scheduled results and saved artifact templates remain under `client.schedule`; run IDs and cursors are opaque, and `artifactSelection: { mode: 'none' }` does not disable Memory-note writing.
