...
test('unified MCP bindings contain all 163 unique tools', () => {
  assert.equal(MCP_TOOL_COUNT, 163)
  assert.equal(new Set(MCP_TOOL_BINDINGS.map(binding => binding.name)).size, 163)
