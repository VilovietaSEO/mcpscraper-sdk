...
The CLI provides friendly shortcuts for common operations plus universal discovery and invocation for every one of the 163 unified MCP tools (76 MCP Scraper plus 87 mirrored memory tools).
...
`tools list` contains exactly 163 manifest-backed names. `tools call` accepts any one of them. Tools marked destructive require `--yes`.
