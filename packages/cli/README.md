...
The CLI provides friendly shortcuts for common operations plus universal discovery and invocation for every one of the 375 unified MCP tools, including provider-neutral X-Ray surveys, attribution-impact reporting, and truthful evidence-status operations.
...
`tools list` contains exactly 375 manifest-backed names. `tools call` accepts any one of them. Tools marked destructive require `--yes`.
