...
The CLI provides friendly shortcuts for common operations plus universal discovery and invocation for every one of the 160 unified MCP tools (73 MCP Scraper plus 87 mirrored memory tools).
...
`tools list` contains exactly 160 manifest-backed names. `tools call` accepts any one of them. Tools marked destructive require `--yes`.
