...
class ProvisionDefaultsInput(BaseModel):
    ...
    grantee_identity: str = Field(..., alias="granteeIdentity", description="Identity that should OWN the 13 default vaults (e.g. an email or user id).")
    issue_key: bool | None = Field(None, alias="issueKey", description="When true, also issue a new API key for the identity entitled to all 13 vaults and return its secret once. Default false.")
...
class ProvisionDefaultsOutput(BaseModel):
    ...
    created: list[str] | None = Field(None, alias="created", description="Vault names newly created on this call (empty when all 13 already existed).")
    vaults: list[str] | None = Field(None, alias="vaults", description="All 13 default vault names the identity now owns.")
