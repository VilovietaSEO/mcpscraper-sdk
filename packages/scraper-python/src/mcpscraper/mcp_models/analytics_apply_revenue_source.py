...
class AnalyticsApplyRevenueSourceInput(BaseModel):
    ...
    confirmed: bool = Field(..., alias="confirmed", description="Confirms this exact provider account, role, and event-family revision.")
    idempotency_key: str = Field(..., alias="idempotencyKey", description="Retry key; reuse only for this exact mutation.")


class AnalyticsApplyRevenueSourceOutput(BaseModel):
    model_config = ConfigDict(populate_by_name=True, extra="allow")
