...
class AnalyticsCreateExportInput(BaseModel):
    ...
    attribution_model: Literal['first_touch', 'last_touch'] | None = Field(None, alias="attributionModel", description="Attribution model applied to the report; defaults to first touch.")
    ...
    report: Literal['overview', 'acquisition', 'content', 'conversions'] | None = Field(None, alias="report", description="Analytics report family to render into the exported artifact.")
    format: Literal['csv', 'json', 'markdown'] | None = Field(None, alias="format", description="Artifact serialization format to generate.")
