...
`client.tools` is the generated 215-tool MCP surface — 114 MCP Scraper tools plus 101 mirrored Memory tools — with one typed snake_case method per tool:
...
This generic compatibility bridge remains available, but new integrations should use `client.tools`, whose generated namespaces provide one typed method for every one of the 215 unified MCP tools.
