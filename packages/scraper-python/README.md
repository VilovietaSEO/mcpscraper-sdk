...
`client.tools` is the generated 373-tool MCP surface, including governed X-Ray revenue, survey, attribution-impact, and evidence-status operations, with one typed snake_case method per tool:
...
This generic compatibility bridge remains available, but new integrations should use `client.tools`, whose generated namespaces provide one typed method for every one of the 373 unified MCP tools.
