...
class CreateScheduledActionToolInput(BaseModel):
    ...
    artifact_selection: Any | None = Field(None, alias="artifactSelection", description="HTML artifact behavior. \"none\" creates no HTML artifact but still permits Memory-note output. \"saved_template\" pins the exact saved template and immutable version IDs.")
...
class CreateScheduledActionToolOutput(BaseModel):
    model_config = ConfigDict(populate_by_name=True, extra="allow")
    ...
    artifact_selection: Any | None = Field(None, alias="artifactSelection", description="The exact stored artifact selection.")
