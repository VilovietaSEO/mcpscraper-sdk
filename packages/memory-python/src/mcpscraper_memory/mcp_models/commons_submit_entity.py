...
class CommonsSubmitEntityInput(BaseModel):
    model_config = ConfigDict(populate_by_name=True, extra="allow")

    idempotency_key: str = Field(..., alias="idempotencyKey", description="Required unique opaque ID for this intended Commons write. Reuse it when retrying the same intended write after a timeout or after correcting a pre-persistence image-ingress failure; use a new value for each intentional create or edit.")
    title: str = Field(..., alias="title", description="Public page title. This is also used for duplicate slug/entity checks when slug is omitted.")
    description: str = Field(..., alias="description", description="Short article summary and schema.org description.")
...
class CommonsSubmitEntityOutput(BaseModel):
    model_config = ConfigDict(populate_by_name=True, extra="allow")
