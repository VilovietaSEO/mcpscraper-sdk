...
class ExtractSiteInput(BaseModel):
    ...
    formats: list[Literal['markdown', 'html', 'links', 'json', 'images', 'branding']] | None = Field(None, alias="formats", description="Requested export views. Every successful page is durably retained as complete JSON, acquired HTML, and Markdown; links and image references are included in page JSON. branding adds a browser-based site summary.")
    background: bool | None = Field(None, alias="background", description="MCP multi-page crawls always run as durable background jobs. Poll check_site_export, then use site_export_read for complete JSON/HTML/Markdown and site_export_image for downloaded images.")
    delivery: Literal['auto', 'artifact'] | None = Field(None, alias="delivery", description="Multi-page crawls are durable exports. auto and artifact both return a job handle followed by an owner-scoped ZIP; artifact explicitly requests that durable destination.")
