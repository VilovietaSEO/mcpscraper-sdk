...
Official Python clients for all 155 unified [mcpscraper.dev](https://mcpscraper.dev) MCP tools plus the direct 86-tool [memory.mcpscraper.dev](https://memory.mcpscraper.dev) API.
...
Use `McpToolsClient(api_key="sk_...")` for all 155 unified tools; it has the same generated namespaces as every other SDK package. Bulk connected-data exports are available as `client.connections.export_connected_service_data(connection_id="conn_123", last_days=7)`; renew expired artifact URLs with `client.connections.renew_connected_data_download(artifact_id="artifact_123")`.
